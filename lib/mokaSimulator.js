...
if (!global.mokaSimulator) {
  global.mokaSimulator = {
    interval: null,
    isRunning: false, // NEW: tracks if simulation is active
    temperature: 20,
    pressure: 0,
    coffeeVolume: 0,
    state: "idle",
...
function startSimulation() {
  ...
  if (sim.isRunning && sim.state !== "finished") {
    console.log("⚠️ Start ignored — Moka pot already running");
    publishAll();
    return;
  }
  ...
  sim.interval = setInterval(() => {
    ...
    if (sim.coffeeVolume >= 100) {
      ...
      sim.state = "finished";
      stopSimulation(true); // hard stop, hold final state
      return;
    }
...
function stopSimulation(fromFinish = false) {
  ...
  if (fromFinish) {
    ...
    sim.isRunning = false; // mark as not running anymore
    publishAll();
    return;
  }
